...
<div class="flex h-screen justify-between items-center">
    ...
    <div class="flex items-center flex-col justify-center w-3/10 h-full mr-40 box-border">
        <h2 class="box-border text-4xl h-1/5 text-center">Zaloguj się do swojego konta klienta</h2>
        <form action="{{route('customer.submitLogin')}}" method="post" class="flex flex-col justify-around items-center w-full h-2/8">
            @csrf
            <label for="email" class="w-full text-left text-l">Adres email</label>
            <input type="email" name="email" id="email" placeholder="email" class="block border-2 border-solid border-blue-800 rounded-full w-full text-3xl box-border pl-4">
            <label for="password" class="w-full text-left text-l">Hasło</label>
            <input type="password" name="password" id="password" placeholder="hasło" class="block border-2 border-solid border-blue-800 rounded-full w-full text-3xl box-border pl-4">
